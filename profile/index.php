...
$stmt = $conn->prepare("SELECT full_name, email, phone, registration_date FROM clients WHERE client_id = ? LIMIT 1");
...
$reg_date     = $client['registration_date'] ? date('d.m.Y', strtotime($client['registration_date'])) : '—';

// Считаем количество заявок
$stmt = $conn->prepare("SELECT COUNT(*) AS total FROM applications WHERE client_id = ?");
$stmt->bind_param("i", $client_id);
$stmt->execute();
$result = $stmt->get_result();
...
// Считаем заявки в работе (не финальные статусы)
$stmt = $conn->prepare("SELECT COUNT(*) AS in_work FROM applications a 
                        LEFT JOIN application_statuses s ON a.status_id = s.status_id 
                        WHERE a.client_id = ? AND (s.is_final = 0 OR s.is_final IS NULL)");
$stmt->bind_param("i", $client_id);
$stmt->execute();
$result = $stmt->get_result();
$in_work = $result->fetch_assoc()['in_work'] ?? 0;
$stmt->close();

// Считаем завершённые заявки
$stmt = $conn->prepare("SELECT COUNT(*) AS done FROM applications a 
                        LEFT JOIN application_statuses s ON a.status_id = s.status_id 
                        WHERE a.client_id = ? AND s.is_final = 1");
$stmt->bind_param("i", $client_id);
$stmt->execute();
$result = $stmt->get_result();
$done = $result->fetch_assoc()['done'] ?? 0;
$stmt->close();
?>
<!DOCTYPE html>
<html lang="ru">
    ...
    <div class="container">
        <div class="profile-grid">
            <div class="profile-sidebar">
                ...
                <a href="../logout.php" style="display:block; margin-top:15px; text-align:center; color:#e74c3c; font-size:14px;">🚪 Выйти</a>
            </div>

            <div class="profile-content">
                <h2>👋 Добро пожаловать, <?php echo htmlspecialchars($client_name); ?>!</h2>
                ...
                <div class="info-item">
                    ...
                    <span class="value"><?php echo htmlspecialchars($client_email); ?></span>
                </div>
                <div class="info-item">
                    <span class="label">📞 Телефон</span>
                    <span class="value"><?php echo htmlspecialchars($client_phone); ?></span>
                </div>
                <div class="info-item" style="border-bottom: none;">
                    <span class="label">📅 Дата регистрации</span>
                    <span class="value"><?php echo $reg_date; ?></span>
                </div>
